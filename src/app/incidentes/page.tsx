...
export default function IncidentesPage() {
  return <AppShell showBack title="Incidentes" eyebrow="Centro de alertas" action={<Link href="/incidents" className="flex h-11 w-11 items-center justify-center rounded-xl border border-border bg-background text-muted-foreground" aria-label="Filtrar alertas"><Clock3 className="h-5 w-5" /></Link>}>
    ...
    <div className="mt-3 space-y-3">{incidents.map((incident) => <Link key={incident.title} href="/incidents" className="block rounded-2xl border border-border bg-card p-4 shadow-sm"><article><div className="flex gap-3"><div className={`mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${incident.tone === "critical" ? "bg-critical/10 text-critical" : incident.tone === "warning" ? "bg-warning/15 text-warning-foreground" : "bg-success/10 text-success"}`}>{incident.tone === "success" ? <Check className="h-5 w-5" /> : <AlertTriangle className="h-5 w-5" />}</div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><div><h3 className="font-semibold leading-5">{incident.title}</h3><p className="mt-1 text-sm text-muted-foreground">{incident.patient}</p></div><ChevronRight className="h-5 w-5 shrink-0 text-muted-foreground" /></div><div className="mt-3 flex items-center justify-between"><span className="text-xs text-muted-foreground">{incident.time}</span><StatusPill tone={incident.tone}>{incident.status}</StatusPill></div></div></div></article></Link>)}</div>
  </AppShell>
}
