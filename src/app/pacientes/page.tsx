...
export default function PacientesPage() {
  return <AppShell showBack title="Pacientes" eyebrow="Seguimiento clínico" action={<Link href="/patients" className="flex h-11 w-11 items-center justify-center rounded-xl border border-border bg-background text-primary" aria-label="Añadir paciente"><Plus className="h-5 w-5" /></Link>}>
    <div className="flex items-center gap-2 rounded-xl border border-border bg-card px-4 py-3"><Search className="h-5 w-5 text-muted-foreground" /><input className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground" placeholder="Buscar paciente" aria-label="Buscar paciente" /><Link href="/patients" className="text-muted-foreground" aria-label="Filtrar pacientes"><SlidersHorizontal className="h-5 w-5" /></Link></div>
    ...
    <div className="mt-3 space-y-3">{patients.map((patient) => <Link key={patient.name} href="/patients" className="block rounded-2xl border border-border bg-card p-4 shadow-sm"><article><div className="flex items-start gap-3"><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-accent text-base font-bold text-primary">{patient.initials}</div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><div><h3 className="font-semibold">{patient.name}</h3><p className="mt-0.5 text-sm text-muted-foreground">{patient.age} · {patient.room}</p></div><StatusPill tone={patient.tone}>{patient.status}</StatusPill></div><p className="mt-3 text-xs text-muted-foreground">{patient.last}</p></div></div></article></Link>)}</div>
  </AppShell>
}
